...
class CIDManager:
    ...
    def __init__(self, config: Configuration, input_filename: str, input_code: SourceCode):
        ...
        self._cid_data = CIDData(source_code_filename=input_filename,
                                 ...
                                 statement_markers_enabled=self._config.statement_analysis_enabled,
                                 decision_markers_enabled=self._config.decision_analysis_enabled,
                                 condition_markers_enabled=self._config.condition_analysis_enabled)
    ...
    def _code_section_inside_parent(self, parent_code_section: CodeSectionData, child_code_section: CodeSectionData) -> bool:
        ...
        if (parent_code_section.start_line > child_code_section.start_line
                or parent_code_section.end_line < child_code_section.end_line):
            return False
        elif (parent_code_section.start_line == child_code_section.start_line
              and parent_code_section.start_column > child_code_section.start_column):
            return False
        elif (parent_code_section.end_line == child_code_section.end_line
              and parent_code_section.end_column < child_code_section.end_column):
            return False
