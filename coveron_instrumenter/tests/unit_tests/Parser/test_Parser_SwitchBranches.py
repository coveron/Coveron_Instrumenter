...
def test_Parser_SwitchBranches_combined_cases(mock_config, mock_cid_manager):
    ...
    assert switch_cases[2].case_type == CaseType.CASE
    assert switch_cases[2].evaluation_code_section == CodeSectionData(
    ...
    assert switch_cases[4].case_type == CaseType.CASE
    assert switch_cases[4].evaluation_code_section == CodeSectionData(
        CodePositionData(16, 5), CodePositionData(16, 11))
    # body code section should be equal to following case
    assert switch_cases[4].body_code_section == switch_cases[3].body_code_section

    # evaluate fifth case result
    assert switch_cases[3].case_type == CaseType.CASE
    assert switch_cases[3].evaluation_code_section == CodeSectionData(
        CodePositionData(17, 5), CodePositionData(17, 11))
    assert switch_cases[3].body_code_section == CodeSectionData(
        CodePositionData(18, 9), CodePositionData(18, 14))

    # evaluate last case result
    assert switch_cases[5].case_type == CaseType.DEFAULT
    assert switch_cases[5].evaluation_code_section == CodeSectionData(
        CodePositionData(19, 5), CodePositionData(19, 12))
    assert switch_cases[5].body_code_section == CodeSectionData(
