...
class Instrumenter:
    ...
    def _prepare_markers(self):
        ...
        if self.config.evaluation_markers_enabled:
            for marker in self.cid_manager.get_evaluation_markers():
                ...
                # store start marker
                self._instrumenter_marker_list.append(InstrumenterMarker(
                                    marker.code_section.start_line,
                                    marker.code_section.start_column,
                ...
                # store end marker
                self._instrumenter_marker_list.append(InstrumenterMarker(
                                    marker.code_section.end_line,
                                    marker.code_section.end_column,
    ...
    def _insert_text(self, insert_text: str, line: int, column: int):
        ...
        source_lines[line - 1] = source_lines[line - 1][:(column -1)] + insert_text + source_lines[line - 1][(column - 1):]
    ...
    def _write_markers(self):
        ...
        for marker in self._instrumenter_marker_list:
            ...
            m_id_1, m_id_2, m_id_3, m_id_4 = marker.marker_id.to_bytes(4, byteorder="big")
            insert_code = ""
            ...
            self._insert_text(insert_code, marker.code_line, marker.code_column)
    ...
    def _write_wrapper(self):
        ...
        source_hash_array = [source_hash_string[i:i+2] for i in range(0, len(source_hash_string), 2)]
        source_hash_array = [("0x"+hexbyte.upper()) for hexbyte in source_hash_array] # add hex signature "0x"
