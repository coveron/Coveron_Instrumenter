...
void test_basic_null_ptr_criFile(void)
{
    ...
    ___COVERON_SET_CHECKPOINT_MARKER(0, 0, 0, 0, &testInputData);

    // check, if the given coveronFile object is set to initialized
    TEST_ASSERT_EQUAL_INT(___COVERON_BOOL_TRUE, testInputData.helperInitialized);
    return;
}
...
void test_basic_fail_file_creation(void)
{
    ...
    // check, if the given coveronFile object is set to not initialized
    TEST_ASSERT_EQUAL_INT(___COVERON_BOOL_FALSE, testInputData.helperInitialized);
}
